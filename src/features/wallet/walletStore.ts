  ...
  devtools(
    persist(
      (set) => ({
        ...
        isDebugMode: false,
        updatePromoCode: (codeName: string | null) =>
          set(() => ({ promoCode: codeName })),
        ...
        reduceCodeBalance: async (codeName: string, amount: number) => {
          try {
            const { data } = await axios.patch(`/api/code/`, {
              code: codeName,
              amount,
            });
            const tons = data[0].tons;
        ...
        updateWalletData: (key: WalletData, value: string | null) =>
          set(() => ({ [key]: value })),
        clearWallet: () =>
          set(() => ({ walletAddress: null, walletPublicKey: null })),
        changeDebugMode: (value: boolean) =>
